...
const OrderProductItem = ({ orderProduct }: OrderProductItemProps) => {
  const productWithTotalPrice = computeProductsTotalPrice(orderProduct.product);
        ...
        <div className="flex w-full items-center justify-between gap-1">
          <div className="felx items-center gap-1">
            ...
            {productWithTotalPrice.discountPercentage > 0 && (
              <p className="text-sx line-through opacity-60">
                R$ {Number(productWithTotalPrice.basePrice).toFixed(2)}
              </p>
            )}
          </div>

          <p className="text-sx opacity-60">Qntd: {orderProduct.quantity}</p>
        </div>
      </div>
    </div>
  );
